...
const ArtistBiographyScreen = ({ route }) => {
  ...
  useEffect(() => {
    if (userId && currentArtistId) {
      ...
      fetch(
        `https://caribeson.com/CONEXION/likes_check.php?usuario_id=${userId}&content_type=artista&content_id=${currentArtistId}`
      )
      ...
      fetch(
        `https://caribeson.com/CONEXION/likes_count.php?content_type=artista&content_id=${currentArtistId}`
      )
        .then((response) => response.json())
        .then((data) => {
      ...
      .then((data) => {
        ...
        if (data.success) {
          ...
          setLikesCount((prevLikesCount) => ({
            ...prevLikesCount,
            [currentArtistId]: isLiked
              ? (prevLikesCount[currentArtistId] || 1) - 1
              : (prevLikesCount[currentArtistId] || 0) + 1,
          }));
          console.log(
            `"Me gusta" ${
              isLiked ? "quitaron" : "añadieron"
            }. Nuevo estado: liked=${!isLiked}, likesCount=${
              isLiked
                ? (likesCount[currentArtistId] || 1) - 1
                : (likesCount[currentArtistId] || 0) + 1
            }`
          );
        } else {
          // Manejar error
          ...
          Alert.alert(
            "Error",
            data.message || 'No se pudo actualizar el "Me gusta".'
          );
        }
      })
      .catch((error) => {
        ...
        Alert.alert(
          "Error",
          'Hubo un problema al actualizar el "Me gusta". Intenta de nuevo.'
        );
      });
  };
  ...
  const onShareArtist = async () => {
    try {
      ...
      Alert.alert(
        "Error",
        "Hubo un problema al intentar compartir. Intenta de nuevo."
      );
      ...
      {item.songs && item.songs.length > 0 ? (
        <FlatList
          ...
          keyExtractor={(song, index) =>
            song.id_cancion
              ? song.id_cancion.toString()
              : `${song.title}-${index}`
          } // Actualizado para usar id_cancion
          renderItem={({ item: song, index }) => (
            <TouchableOpacity
              style={styles.songItem}
              onPress={() => {
                console.log("El valor de song.id_cancion es:", song);

                if (song.id_cancion) {
                  navigation.navigate("VideoFromYouTube", {
                    youtubeLink: song.link,
                    title: song.title,
                    songId: song.id_cancion, // Aquí usamos id_cancion
                  });
                } else {
                  Alert.alert(
                    "Error",
